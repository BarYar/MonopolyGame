...
class Board(Root):
    ...
    def __init__(self):
        ...
        self.roll_dice_pic = tk.PhotoImage(file=f'{self.project_root}\Files\Misc\dice.png')
    ...
    def create_squares(self):
        ...
        background = tk.Label(self.board, bg="white")
        background.place(x=0.2*height, y=0.2*height, height=0.6*height, width=0.6*height)
        result1 = random.randint(1, 6)
        result2 = random.randint(1, 6)
        results = tk.StringVar(value=f" {result1}                {result2} ")
        dice_result = tk.Label(self.getBoard(), image=self.roll_dice_pic, textvariable=results,
                               font=(None, 16,'bold'), compound='center', bg="white")
        dice_result.place(x=0.425*height, y=0.225*height, height=0.125*height, width=0.158*height)
        self.monopoly_title = tk.PhotoImage(file=f'{self.project_root}\Files\Misc\monopoly_title.png')
        title = tk.Label(self.board, image=self.monopoly_title, bg= "white")
        title.place(x=0.2*height, y=0.4*height, height=0.15*height, width=0.6*height)
        #For testing
